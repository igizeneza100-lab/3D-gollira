...
export default function Navbar() {
  ...
  return (
    <div className="fixed top-0 left-0 right-0 z-50">
      {/* Top Banner - Compact & High-Density */}
      <div className={cn(
        "bg-dark-charcoal text-white/70 transition-all duration-300 overflow-hidden border-b border-white/5",
        isScrolled ? "max-h-0 opacity-0" : "max-h-12 opacity-100"
      )}>
        <div className="max-w-7xl mx-auto px-6">
          <div className="h-9 flex items-center justify-between text-[11px] font-medium tracking-wider">
            {/* Socials */}
            <div className="hidden lg:flex items-center gap-2">
              <a href="#" className="w-7 h-7 flex items-center justify-center rounded-full border border-white/10 hover:bg-brand-yellow hover:text-dark-charcoal transition-all">
            ...
            <div className="flex-1 overflow-hidden mx-8">
              ...
              >
                <span>Pioneer in Digital Rehabilitation & 3D Design</span>
                <span className="hidden md:inline w-1 h-1 rounded-full bg-white/20" />
                <span className="hidden md:inline">Innovation Hub Kigali</span>
              </motion.div>
            </div>

            {/* Quick Contact */}
            <div className="hidden lg:flex items-center gap-6">
              <a href="tel:+250783723705" className="flex items-center gap-2 hover:text-brand-yellow transition-colors">
                <Phone className="w-3.5 h-3.5" />
                +250 783 723 705
              </a>
              <a href="mailto:info@gorilla3d.com" className="flex items-center gap-2 hover:text-brand-yellow transition-colors">
